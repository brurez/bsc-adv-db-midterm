...
function App() {
  ...
  return (
    <div className="App container">
      ...
      <h3 className="is-size-4 mt-4">Composers with more music pieces</h3>
      <div className="box p-6">
        <svg viewBox="0 0 50 15" xmlns="http://www.w3.org/2000/svg">
          {topComposers.map((item, index) => {
            const height = 15 * item.compositions / topComposers[0].compositions;
            return (
              <>
              <rect
                x={index * 5}
                ...
                rx="1"
              />
              <text fontSize={1} x={index * 5 + 0.5} y={13.5}>{item.lastname}</text>
              <text fontSize={1} x={index * 5 + 0.5} y={14.5}>{item.compositions}</text>
              </>
            )
          })}
        </svg>
      ...
      <button className="button mb-4 is-primary" onClick={handleLoadMoreClick}>
        Load more
      </button>
    </div>
  );
